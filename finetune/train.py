...
def train(
    ...
    train_dataloader = get_dataloader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, worker_init_fn=lambda worker_id: np.random.seed(args.seed))
    if args.max_steps > 0:
...
def main():
    ...
    # load data
    train_dataset = torch.load(os.path.join(args.data_dir, "train_{}.pt".format("roberta-large_v2")))
    eval_dataset = torch.load(os.path.join(args.data_dir, "valid_{}.pt".format("roberta-large_v2")))
